...
val migration_1_2 = object : Migration(1, 2) {
    override fun migrate(database: SupportSQLiteDatabase) {
       database.execSQL(
            "ALTER TABLE PiratePlace ADD COLUMN hasVisited INTEGER NOT NULL DEFAULT 0"
        )
    database.execSQL(
    "ALTER TABLE PiratePlace ADD COLUMN latitude REAL NOT NULL DEFAULT 0.00"
    )
     database.execSQL(
      "ALTER TABLE PiratePlace ADD COLUMN longitude REAL NOT NULL DEFAULT 0.00"
        )
}
}